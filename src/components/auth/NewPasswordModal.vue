...
export default defineComponent({
	...
	methods: {
		checkShouldOpen() {
			if (this.$route.query.passToken) {
				this.openModal('new-password-modal');
			}
		},
		submitForm() {
...
<template>
	<BaseModal id="new-password-modal" v-slot="{close}" @close="$router.push('/')">
		<section class="modal">
			...
			<p class="modal__text">Придумайте новый пароль.</p>
			...
			<form class="modal__form form" @submit.prevent="submitForm">
				<div class="form__inputs">
					<div class="form__input">
						<InputText id="recover-password-new-password" v-model="formData.password" :errors="v$.formData.password.$errors" label="Придумайте новый пароль" placeholder="Введите новый пароль" password />
					</div>
					<div class="form__input">
						<InputText id="recover-password-repeat-password" v-model="formData.confirmPassword" :errors="v$.formData.confirmPassword.$errors" label="Повторите новый пароль" placeholder="Введите новый пароль еще раз" password />
					</div>
				</div>
				<div class="form__bottom">
					<p class="form__policy form__policy--grey">
						Нажимая кнопку «Сохранить», Вы&nbsp;<a :href="getLegalDocs.processingPersonal" target="_blank">соглашаетесь</a> с&nbsp;условиями <a :href="getLegalDocs.privacyPolicy" target="_blank">политики обработки персональных данных</a>.
					</p>
			...
			<NotionModal id="new-password-notion" title="Ваш пароль успешно изменен" text="Теперь вы можете использовать данный пароль для входа в свой профиль на сайте." @close="close"/>
